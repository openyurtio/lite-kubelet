...
func (s *ObjectSourceFile) listConfig() error {
	...
	case statInfo.Mode().IsRegular():
		klog.V(5).Infof("listConfig: find path %s is regular, create watchfile event", path)
...
func (s *ObjectSourceFile) extractFromDir(name string) error {
	...
	for _, path := range dirents {
		...
		case statInfo.Mode().IsRegular():
			klog.V(5).Infof("extractFromDir: find path %s is regular, create watchfile event", path)
			s.watchEvents <- &watchFileEvent{path, FileListAdd}
		default:
...
func (s *ObjectSourceFile) produceWatchEvent(e *fsnotify.Event) error {
	...
	klog.V(5).Infof("produceWatchEvent: name %s, type %s", e.Name, eventType)
